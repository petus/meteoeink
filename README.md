...
Čidlo se detekuje samo, ale **bez nastavení nebude měřit správně.** Projděte tyhle kroky, jinak dostanete nesmyslné CO₂ a podhodnocený prach.
...
- **Deklarovaná přesnost platí až po dlouhém souvislém běhu** — 12 h u SEN63C, 2 dny u SEN66. Toho při provozu na baterii nedosáhnete.

Jediná fungující cesta je jednorázová ruční kalibrace:
...
Trend CO₂ (vyvětráno / dusno) je po kalibraci spolehlivý, absolutní hodnotu ber s rezervou. **První měření po nahrání firmwaru není směrodatné.**
...
- ⚠️ **Arduino IDE vkládá vygenerované prototypy těsně před první definici funkce v souboru.** Když se typ použitý v hlavičce funkce (`Quantity`, `Reading`, `Detected`, `SenKind`, …) definuje až za tou první funkcí, překlad spadne na desítkách hlášek `'Quantity' was not declared in this scope`. **V souboru nesmí být žádná definice funkce dřív než blok typů v sekci VELIČINY A ČIDLA.** Čistý `g++` tuhle chybu neodhalí.
...
toff.<čidlo>=<x>  offset teploty [°C]   (-20 … 20), čidlo: sht sen scd bme ds
hoff.<čidlo>=<x>  offset vlhkosti [%RH] (-30 … 30), čidlo: sht sen scd bme
...
ČIDLO PRACHU SEN6x
senwarm=<s> doba běhu před odečtem (5 … 120 s, výchozí 30)
senmult=<n> násobek intervalu pro PM a CO₂ (1 … 4)
senauto=<d> automatické čištění ventilátoru po d dnech (0 … 90, 0 = vypnuto)
...
MĚŘENÍ A GRAF
int=<min>     základní interval (1 … 60) — mění délku historie
